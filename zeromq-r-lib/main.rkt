...
;; A Socket is (socket (U _zmq_socket-pointer #f) Mutex (Listof Endpoint))
;; A Endpoint is (cons 'bind String) | (cons 'connect String)
(struct socket ([ptr #:mutable] wmutex [ends #:mutable])
...
(define (zmq-socket type
  ...
  (define sock (socket ptr (make-mutex) null))
...
(struct recv-evt (sock)
  ...
  (unsafe-poller
   (lambda (self wakeups)
     ...
     (define ptr (socket-ptr sock))
     (cond [(not ptr)
            (values (list (lambda (who) (error who "socket is closed\n  socket: ~e" sock))) #f)]
